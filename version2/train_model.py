...
from load_dota_matches import CSV_FILE, get_hero_data
...
def create_input_vector(team_picks, opponent_picks, actual_pick):
    ...
    return np.concatenate([team_vec, opponent_vec, pick_vec])
...
def prepare_training_data(decisions, test_size=0.2):
    ...
    print(f"Augmented dataset size before split: {len(X)}")
...
def optimize_hyperparameters(x_train, y_train, x_val, y_val, input_size):
    ...
    for params in ParameterGrid(param_grid):
        ...
        val_pred = estimator.predict(x_val_tensor)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Dota Hero Predictor Model",
    )
    parser.add_argument(
        "--optimize",
        action="store_true",
    ...
    try:
        decisions = load_decisions_from_csv(CSV_FILE)
